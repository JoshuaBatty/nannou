...
pub use self::camera::{Camera, CameraController, Projection};
...
impl Draw {
    ...
    /// Produce a new **Draw** instance that will draw with a perspective projection.
    pub fn camera(&self, camera: Camera) -> Self {
        let mut context = self.context.clone();
        context.camera = Projection::Perspective(camera);
        self.context(context)
    }

    /// Produce a new **Draw** instance that will draw with the given camera.
    pub fn projection(&self, projection: Projection) -> Self {
        let mut context = self.context.clone();
        context.camera = projection;
        self.context(context)
    }

    /// Produce a new **Draw** instance that will draw with an orthographic projection.
    pub fn orthographic(&self) -> Self {
        let mut context = self.context.clone();
        context.camera = Projection::Orthographic;
        self.context(context)
    }
