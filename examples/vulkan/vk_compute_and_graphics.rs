...
fn main() {
    nannou::app(model)
        ...
        .update(update) // The function that will be called when the app receives events.
...
fn model(app: &App) -> Model {
    ...
    let vertex_buffer = {
        ...
        let data_iter = (0 .. 1024).map(|n| Vertex{position: [0.0; 3]});
...
// https://www.vertexshaderart.com/art/PHANwhhRXk62fsaeC
...
void main() {
    ...
    if(mod(idx,3) == 0) {
        data.data[idx] = vec3(0.0);
        return;
    }
    data.data[idx] = vec3(sin(idx * lfo + pc.time * 1000.0), cos(idx * -lfo + pc.time * -1000.0), cos(idx * -lfo + pc.time * -2000.0));
}"
    }
}
